...
/* ── Main component ─────────────────────────────────────────────────────────── */
export default function ResultsSection({ sortResults, onDownloadPDF, onSortAgain, targetCollection, onSaveNotes }) {
    ...
    <section className="animate-fade-up">
      ...
      {targetCollection ? (
        <div className="flex items-center justify-between gap-4 mb-5 px-5 py-4 bg-blue-50 border border-blue-200 rounded-2xl shadow-sm">
          ...
          <button onClick={() => onSaveNotes(images)} disabled={images.length === 0}
        ...
        <div className="flex items-center justify-between gap-4 mb-5 px-5 py-4 bg-green-50 border border-green-200 rounded-2xl shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[13px] font-bold text-gray-900">Save to <span className="text-green-600">My Notes</span></p>
              <p className="text-[11px] text-gray-500">
                {images.length} page{images.length !== 1 ? 's' : ''} will be saved as uncategorized notes. You can assign them to a collection later.
              </p>
            </div>
          </div>
          <button onClick={() => onSaveNotes(images)} disabled={images.length === 0}
            className="flex items-center gap-2 px-5 py-2.5 bg-green-600 hover:bg-green-700 text-white text-[13px] font-bold
                       rounded-full shadow transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed flex-shrink-0">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
            </svg>
